...
import com.cg.training.model.WithdrawDepositReq;
...
public class AccountServiceImpl implements AccountService {
	...
	public Account createAccount(final AccountWrapper account) {
		...
		final Optional<Customer> cust = custRepo.findById(account.getcId());

		final Customer customer = cust.get();

		final Optional<Bank> bankOpt = bankRepo.findById(account.getbId());
		...
		final BigDecimal newAccAmount = account.getAccount().getAmount();
	...
	public String withdrawMoney(final WithdrawDepositReq withdraw) {
		...
		// Optional<Bank> obj = bankRepo.findById(bankid);
